...
if (newDate.toLowerCase() === 'null') {
  // If the argument is 'null', bypass the date checks and proceed to update
  updateConfig('null');
} else {
...
function updateConfig(newValue) {
  fs.readFile(configFilePath, 'utf8', (err, data) => {
    ...
    if (newValue === 'null') {
      ...
      updatedData = data.replace(/(NEXT_DELIVERY_DATE\s*=\s*)[^\;]+/, `$1null`);
    } else {
      // Replace the NEXT_DELIVERY_DATE value with the new date object
      updatedData = data.replace(/(NEXT_DELIVERY_DATE\s*=\s*)[^\;]+/, `$1${newValue}`);
    }
